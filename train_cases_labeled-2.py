    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        for view in view_types:
            ...
            try:
                ...
                dummy_img = torch.randn(dummy_tensor_shape)
...
class MultiViewMammogramModel(nn.Module):
    def __init__(self, num_classes: int = NUM_CLASSES, dropout_rate: float = 0.3):
        ...
        self.resnet_last_layer = nn.Linear(num_features, num_features)
        ...
        text_features = self.bert.config.hidden_size   # = 768
    ...
    def forward(self, images: torch.Tensor, input_ids: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
        ...
        features = self.resnet_last_layer(features)   
        features = features.view(B, V, -1)            
        image_features = features.reshape(B, -1)      
...
class BreastCancerDetector:
    def __init__(self, data_dir: str, batch_size: int = 2, learning_rate: float = 1e-5):
        self.data_dir = data_dir
    ...
    def load_data(self):
        ...
        train_val_cases, test_cases, _, _ = train_test_split(
        ...
        train_cases, val_cases, _, _ = train_test_split(
            train_val_cases, 
            [all_labels[self.cases.index(c)] for c in train_val_cases],  
            test_size=0.2,          
            random_state=42,
            stratify=None
        )
        ...
        self.val_dataset   = MammogramDataset(self.data_dir, val_cases, self.transform)
        self.test_dataset  = MammogramDataset(self.data_dir, test_cases, self.transform)
        
        print(f"Loaded {len(train_cases)} training cases, {len(val_cases)} validation cases, and {len(test_cases)} test cases")
    ...
    def create_model(self):
        ...
        counts = np.bincount(self.train_dataset.labels)
...
"""
    ...
    def plot_training_history(self, train_losses: List[float], train_accuracies: List[float],
                              val_losses: List[float] = None, val_accuracies: List[float] = None):
        ...
        print("Training/validation history plot saved to training_validation_history.png")
    ...
    def save_model(self, path: str = 'breast_cancer_detector.pth'):
        ...
        torch.save({
            ...
            'scaler_state_dict': self.scaler.state_dict(),   # save scaler for resume
        }, path)
...
def main():
    ...
    detector = BreastCancerDetector(data_dir=data_dir_path, batch_size=12, learning_rate=1e-5)
